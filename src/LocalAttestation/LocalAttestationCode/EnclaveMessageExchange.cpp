...
extern "C" {
	...
	uint32_t enclave_to_enclave_call_dispatcher(char* decrypted_data,
		size_t decrypted_data_length, char** resp_buffer, size_t* resp_length);
	uint32_t message_exchange_response_generator(char* decrypted_data,
...
#pragma warning (pop)
	...
	//Ocall to request for a session with the destination enclave and obtain session 
	//id and Message 1 if successful
	status = session_request_ocall(&retstatus, src_enclave_id, dest_enclave_id,
		&dh_msg1, &session_id);
	...
	status = exchange_report_ocall(&retstatus, src_enclave_id, dest_enclave_id,
	...
	uint32_t *session_id)
{
	...
	g_dest_session_info_map.insert(std::pair<sgx_enclave_id_t, 
		dh_session_t>(src_enclave_id, session_info));
	...
	uint32_t session_id)
{
	...
	do
	{
		...
		std::map<sgx_enclave_id_t, dh_session_t>::iterator it =
			g_dest_session_info_map.find(src_enclave_id);
	...
	size_t* out_buff_len)
{
	...
	status = sgx_rijndael128GCM_encrypt(&session_info->active.AEK, 
		(uint8_t*)inp_buff, data2encrypt_length,
	...
	*out_buff = (char*)malloc(max_out_buff_size);
	...
	status = send_request_ocall(&retstatus, src_enclave_id, dest_enclave_id, 
		req_message, (sizeof(secure_message_t) + inp_buff_len), max_out_buff_size,
	...
	if (*(resp_message->message_aes_gcm_data.reserved) != 
	...
	size_t resp_message_size)
{
	...
	//Casting the decrypted data to the marshaling structure type to obtain type of
	//request (generic message exchange/enclave to enclave call)
	...
	else if (ms->msg_type == ENCLAVE_TO_ENCLAVE_CALL)
	{
		...
		ret = enclave_to_enclave_call_dispatcher((char*)decrypted_data, 
			decrypted_data_length, &resp_data, &resp_data_length);
		if (ret != 0)
		{
			SAFE_FREE(decrypted_data);
			SAFE_FREE(resp_data);
	...
	status = sgx_rijndael128GCM_encrypt(&session_info->active.AEK, 
		(uint8_t*)resp_data, data2encrypt_length,
